    ...
    def _run_steps(
        ...
        if self.ai_package.get("script_blocks"):
            ...
            try:
                ...
                pass
        # ── SCRIPT REVIEW (tùy chọn) ─────────────────────────────────────────
        # Nếu có script_review_callback → hiện Script Editor cho user chỉnh sửa
        # trước khi tạo voice. Callback trả False = user huỷ pipeline.
        if self.script_review_callback and callable(self.script_review_callback):
            ...
            if not proceed:
                ...
                return False
            # Reload ai_package sau khi user chỉnh sửa (đã lưu lại file)
            pkg_file = os.path.join(self.output_dir, "ai_package.json")
            ...
            self._log("   🚀 Chế độ tự động: tiếp tục tạo voice, phụ đề và xuất video.")
        # ── END SCRIPT REVIEW ────────────────────────────────────────────────
